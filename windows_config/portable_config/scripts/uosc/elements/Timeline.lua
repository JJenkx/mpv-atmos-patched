...
---Format a byte/second rate as a compact human string, e.g. "4.3 MiB/s".
---@param bps number|nil bytes per second
---@return string
local function format_speed(bps)
	bps = bps or 0
	if bps >= 1048576 then
		return string.format('%.1f MiB/s', bps / 1048576)
	elseif bps >= 1024 then
		return string.format('%.0f KiB/s', bps / 1024)
	else
		return string.format('%d B/s', bps)
	end
end

---Format per-worker download rates as compact space-separated MiB/s numbers,
---e.g. "2.1 2.0 1.9 2.0". Returns nil when there are no workers.
---@param speeds number[]|nil per-connection bytes/sec (segmented-worker-rates)
---@return string|nil
local function format_worker_speeds(speeds)
	if not speeds or #speeds == 0 then return nil end
	local parts = {}
	for i, bps in ipairs(speeds) do
		parts[i] = string.format('%.1f', (bps or 0) / 1048576)
	end
...
function Timeline:render()
	...
	if state.uncached_ranges then
		...
		for _, range in ipairs(state.uncached_ranges) do
			if options.timeline_cache then
				...
				opts.color, opts.opacity, opts.anchor_x = 'ffffff', 0.6 - (0.25 * visibility), bax
				ass:texture(ax, fay, bx, fby, texture_char, opts)
				opts.color, opts.opacity, opts.anchor_x = '000000', 0.75 - (0.2 * visibility), bax + offset
				ass:texture(ax, fay, bx, fby, texture_char, opts)
			end
		end
	end
	...
	-- Bright-white two-pass draw for the download-speed readout: pure white over
	-- the (dark) unplayed region, dark over the (white) progress bar so it stays
	-- legible wherever the playhead is.
	local function draw_speed_text(x, y, align, text, opts)
		opts.color, opts.border_color = bg, fg
		opts.clip = '\\clip(' .. foreground_coordinates .. ')'
		ass:txt(x, y, align, text, opts)
		opts.color, opts.border_color = 'ffffff', bg
		opts.clip = '\\iclip(' .. foreground_coordinates .. ')'
		ass:txt(x, y, align, text, opts)
	end
	...
	if text_opacity > 0 then
		...
		-- Download speed (segmented downloader). Replaces the old buffered-seconds
		-- readout. state.download_speed is the combined bytes/sec of the download
		-- worker threads (nil for local / non-segmented streams -> hidden). Each
		-- file is a column: combined rate on top, per-thread rates under it. When
		-- the next playlist entry is prefetching in parallel, its column is drawn
		-- to the right of the current file's, with a vertical divider between.
		if state.download_speed ~= nil then
			local margin = 5 * state.scale
			local fs = self.font_size
			local line_gap = 2 * state.scale
			local border = options.text_border * state.scale
			-- Build one file's column of stacked lines (total on top, threads under).
			local function build_column(speed, workers, total_size, total_op, worker_size, worker_op)
				local col = {lines = {}, width = 0, height = 0}
				local function add(text, size, op)
					local opts = {size = size, opacity = text_opacity * op, border = border}
					col.lines[#col.lines + 1] = {text = text, size = size, opts = opts}
					col.width = math.max(col.width, text_width(text, opts))
					col.height = col.height + size + (#col.lines > 1 and line_gap or 0)
				end
				add(format_speed(speed), total_size, total_op)
				local w = format_worker_speeds(workers)
				if w then add(w, worker_size, worker_op) end
				return col
			end
			local cur = build_column(state.download_speed, state.worker_speeds, fs * 0.8, 1, fs * 0.55, 0.9)
			local pre = state.prefetch_download_speed ~= nil
				and build_column(state.prefetch_download_speed, state.prefetch_worker_speeds, fs * 0.7, 0.9, fs * 0.5, 0.8)
				or nil
			-- Horizontal placement: current column, then a divider + prefetch column.
			local col_gap = 10 * state.scale
			local total_w = cur.width + (pre and (col_gap + pre.width) or 0)
			local x = fbx + margin
			...
			if x < min_x then x = min_x elseif x + total_w > max_x then x = math.max(min_x, max_x - total_w) end
			-- Draw a column left-aligned at cx, vertically centered on the bar (shift
			-- up if it would spill below the bar onto the controls).
			local function draw_column(col, cx)
				local y = fcy - col.height / 2
				local overflow = (y + col.height) - fby
				if overflow > 0 then y = y - overflow end
				for _, l in ipairs(col.lines) do
					draw_speed_text(cx, y + l.size / 2, 4, l.text, l.opts)
					y = y + l.size + line_gap
				end
			end
			draw_column(cur, x)
			if pre then
				-- Vertical divider between the two files, centered on the bar.
				local sep_h = math.max(cur.height, pre.height)
				local sep_x = x + cur.width + col_gap / 2
				local sep_y = fcy - sep_h / 2
				ass:rect(sep_x, sep_y, sep_x + math.max(1, state.scale), sep_y + sep_h,
					{color = 'ffffff', opacity = text_opacity * 0.5})
				draw_column(pre, x + cur.width + col_gap)
			end
		end
