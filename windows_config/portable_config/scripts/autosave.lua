...
local function init()
  ...
  if path and wl_dir then
    ...
    -- Windows: certutil ships with the OS (no md5sum here).
    local tmpdir  = os.getenv("TEMP") or os.getenv("TMP") or "."
    local tmpfile = tmpdir .. "\\mpv_autosave_" .. math.random(1e6, 9e6)
    local fw = io.open(tmpfile, "wb")
    if fw then
      fw:write(path)
      fw:close()
      local res = mp.command_native({
        args           = {"certutil", "-hashfile", tmpfile, "MD5"},
        capture_stdout = true,
        playback_only  = false,
      })
      os.remove(tmpfile)
      if res and res.stdout then
        -- certutil prints the hash on the line after the header; older
        -- Windows versions space-separate the hex bytes.
        local line = res.stdout:match("\r?\n([0-9a-fA-F%s]+)\r?\n")
        local hash = line and line:gsub("%s+", "")
