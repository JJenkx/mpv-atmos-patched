...
local function cache_stats()
    ...
    -- Segmented downloader: true combined network rate + per-connection breakdown
    -- (patched mpv only; absent for local/non-segmented streams).
    local seg_speed = info["segmented-input-rate"]
    if seg_speed ~= nil then
        append(stats, utils.format_bytes_humanized(seg_speed) .. "/s",
               {prefix = "Segmented DL:"})
        local workers = info["segmented-worker-rates"]
        if workers ~= nil and #workers > 0 then
            local parts = {}
            for i, w in ipairs(workers) do
                parts[i] = string.format("w%d %s", i - 1, utils.format_bytes_humanized(w) .. "/s")
            end
            append(stats, table.concat(parts, "  "), {prefix = "Workers:"})
        end
    end
