...
local options = {
    ...
    -- Generate thumbnails from mpv's in-memory demuxer cache via the built-in
    -- thumbnail-cache command (requires the thumbnail_cache mpv patch) instead
    -- of spawning a second mpv. No subprocess fork (avoids the spdif-underrun
    -- wedge) and no extra network I/O; only already-buffered seek positions get
    -- a thumbnail. Implies network playback support.
    cache_backend = false,
...
-- cache_backend needs the patched mpv "thumbnail-cache" command; feature-detect
-- so a stock mpv silently falls back to the normal subprocess thumbnailer.
local cache_backend = options.cache_backend
if cache_backend then
    local cmds = mp.get_property_native("command-list")
    local found = false
    if cmds then
        for _, c in ipairs(cmds) do
            if c.name == "thumbnail-cache" then found = true break end
        end
    end
    if not found then
        mp.msg.warn("thumbfast: cache_backend enabled but mpv lacks the thumbnail-cache command; falling back to subprocess thumbnailer")
        cache_backend = false
    end
end
...
local function info(w, h)
    ...
    disabled = (w or 0) == 0 or (h or 0) == 0 or
        has_vid == 0 or
        (properties["demuxer-via-network"] and not options.network and not cache_backend) or
...
local function spawn(time)
    ...
    -- Cache backend: no subprocess. Frames are pulled from the demuxer cache by
    -- the thumbnail-cache command in seek(); just mark ourselves "spawned" so
    -- the normal thumb()/file-poll flow proceeds.
    if cache_backend then
        has_vid = properties["vid"] or 0
        spawned = true
        spawn_waiting = false
        spawn_working = true
        return
...
local function run(command)
    ...
    if cache_backend then return end
...
-- cache_backend request coalescing: keep at most one thumbnail-cache command in
-- flight and only ever remember the latest requested time. Without this, fast
-- panning queues dozens of async commands that then all decode+draw in sequence,
-- so you watch a backlog drain when the cursor lands.
local cache_cur = nil      -- time currently being decoded (in flight), or nil
local cache_pending = nil  -- latest desired time waiting for the in-flight one

local function cache_issue(t)
    cache_cur = t
    mp.command_native_async(
        {"thumbnail-cache", t, effective_w, effective_h, options.thumbnail},
        function()
            local done = cache_cur
            cache_cur = nil
            local nextt = cache_pending
            cache_pending = nil
            if nextt ~= nil and nextt ~= done then
                cache_issue(nextt)
            end
        end)
end
...
local function seek(fast)
    ...
    if cache_backend then
        -- Decode the keyframe into the overlay file. The command writes exactly
        -- effective_w*effective_h*4 BGRA bytes, matching what check_new_thumb()/
        -- real_res()/overlay-add expect. If nothing is available it writes
        -- nothing and the last thumbnail stays.
        local t = last_seek_time
        if cache_cur == nil then
            cache_issue(t)
        elseif t ~= cache_cur then
            cache_pending = t   -- collapse to the newest position only
        end
        return
    end
...
local function clear()
    ...
    last_x = nil
    last_y = nil
    cache_pending = nil   -- drop any queued cache_backend request
...
local function file_load()
    ...
    last_tone_mapping = nil
    last_seek_time = nil
    cache_cur = nil       -- reset cache_backend coalescing for the new file
    cache_pending = nil
    if info_timer then
        info_timer:kill()
        info_timer = nil
