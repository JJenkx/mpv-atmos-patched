...
state = {
	...
	cache_duration = nil,
	download_speed = nil, -- segmented downloader combined bytes/sec (nil = not a segmented stream)
	worker_speeds = nil, -- segmented downloader per-connection bytes/sec array
	prefetch_download_speed = nil, -- next-file prefetch combined bytes/sec (nil = not prefetching a segmented stream)
	prefetch_worker_speeds = nil, -- next-file prefetch per-connection bytes/sec array
	core_idle = false,
	eof_reached = false,
...
mp.observe_property('demuxer-cache-state', 'native', function(prop, cache_state)
	...
	if cache_state then
		...
		set_state('download_speed', cache_state['segmented-input-rate'])
		set_state('worker_speeds', cache_state['segmented-worker-rates'])
		set_state('prefetch_download_speed', cache_state['segmented-prefetch-input-rate'])
		set_state('prefetch_worker_speeds', cache_state['segmented-prefetch-worker-rates'])
	else
		cached_ranges = {}
		set_state('download_speed', nil)
		set_state('worker_speeds', nil)
		set_state('prefetch_download_speed', nil)
		set_state('prefetch_worker_speeds', nil)
	end
...
-- Keep the segmented download-speed readouts live even when demuxer-cache-state
-- stops firing (e.g. once a file is fully buffered to EOF, mpv stops ticking the
-- cache updater). Poll ~1s; the patched mpv recomputes the rate on each read and
-- decays it to 0 shortly after the download workers go idle. Only pushes state
-- (and a render) when a value actually changes, so it's free on local files.
mp.add_periodic_timer(1, function()
	local cs = mp.get_property_native('demuxer-cache-state')
	local speed = cs and cs['segmented-input-rate'] or nil
	local pspeed = cs and cs['segmented-prefetch-input-rate'] or nil
	if speed ~= state.download_speed or pspeed ~= state.prefetch_download_speed then
		set_state('download_speed', speed)
		set_state('worker_speeds', cs and cs['segmented-worker-rates'] or nil)
		set_state('prefetch_download_speed', pspeed)
		set_state('prefetch_worker_speeds', cs and cs['segmented-prefetch-worker-rates'] or nil)
		request_render()
	end
end)
